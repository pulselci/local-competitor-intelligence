...
def compute_snapshot_deltas(business_id: UUID, days: int = 30) -> list[dict]:
    """
    Latest day snapshot per competitor + 1d/7d/30d deltas (rating + review_count).
    Fetches an extra 35 days of history so lag(30) has enough rows to look back through.
    Returns list[dict].
    """
    sql = """
    with daily as (
      ...
      where s.business_id = %s
        and s.observed_at >= (now() at time zone 'utc') - ((%s::int + 35) || ' days')::interval
    ),
    d as (
      select
        business_id,
        ...
        lag(google_review_count, 30) over (partition by business_id, competitor_id order by day_utc) as reviews_30d_ago
      from daily
      where rn = 1
    ),
    ...
    select
      ...
      (d.google_review_count - d.reviews_30d_ago) as reviews_delta_30d
