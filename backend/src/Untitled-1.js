...
function formatSnapshotTime(iso) {
  ...
  // ✅ NEW: delete currently selected snapshot (A2)
    ...
    try {
      ...
      await api.deleteSnapshot(snapshotId);
        ...
        // remember where the deleted snapshot was in the CURRENT list
      ...
      // if we deleted the currently selected snapshot, select the closest neighbor
      if (selectedSnapshotId === snapshotId) {
        if (snaps.length === 0) {
          setSelectedSnapshotId(null);
          setSelectedSnapshotDetail(null);
        } else {
          // choose the item that shifted into the same index; if deleted last, choose new last
  ...
  async function deleteSelectedSnapshot() {
    ...
    try {
      ...
      // clear selection + detail
        // refresh snapshot list for current business
      const snaps = await api.snapshots(selectedId);
      setSnapshots(snaps);

      if (snaps.length > 0) {
        // auto-select newest (list is newest-first)
        ...
        snapshots: [
          {
            ...
            visibility_score: formData.visibility_score
              ? Number(formData.visibility_score)
              : null,
  ...
  snapshotCompetitorFilter === "all"
    ? snapshots
    : snapshots.filter(
        (s) => s.competitor_id === snapshotCompetitorFilter
      );

  const snapshotPayloadTemplate = useMemo(() => {
    // Generates sample snapshots: 3 observations per competitor at different times
...
async function submitReport() {
  ...
  <select
    ...
    onChange={(e) => {
      ...
      // If the currently selected snapshot isn't in the new filter, clear it
      if (v !== "all") {
                  ...
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData((p) => ({ ...p, google_rating: e.target.value }))}
                        placeholder="4.2"
                        style={{
                          width: 110,
                          padding: "8px 10px",
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setFormData((p) => ({ ...p, price_hint: e.target.value }))}
                        style={{
                          width: 110,
                          padding: "8px 10px",
                          borderRadius: 8,
  ...
  "source": "manual"
                ...
                {filteredSnapshots.length ? (
                  ...
                  <div style={{ color: "#666" }}>
                    No snapshots yet. Click <strong>Add sample snapshots</strong> to generate demo
                    data.
                  </div>
                )}
              </section>
                  ...
                  <>
                    {latestReport.signed_url_placeholder ? (
                      <div style={{ marginBottom: 8 }}>
                        <a
                          href={latestReport.signed_url_placeholder}
                          target="_blank"
                          rel="noreferrer"
                        >
                  ...
                  <ul style={{ paddingLeft: 16, marginTop: 6 }}>
                    {reports.map((r) => (
                      <li key={r.id} style={{ marginBottom: 8 }}>
                        ...
                        <div style={{ fontSize: 12, fontFamily: "monospace", color: "#777" }}>
                          {r.id}
                        </div>
                      </li>
                    ))}
