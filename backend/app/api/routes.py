...
def _build_delta_recs(delta: dict, sections: dict, business_name: str = "") -> list[dict]:
    ...
    if next_target:
        ...
        if owner_gained >= realistic_monthly:
            ...
            _rw = "review" if owner_gained == 1 else "reviews"
            action = f"You gained {owner_gained} {_rw} last month — {'ahead of' if owner_gained > realistic_monthly else 'on'} pace."
            detail = (
...
def _build_data_driven_execution_plan(sections: dict, business_name: str = "") -> list[dict]:
    ...
    if owner and leader:
        ...
        if owner_reviews >= leader_reviews:
            ...
            # Find the next competitor just above the owner — the achievable near-term target
            next_above = min(
                [r for r in competitors if int(r.get("reviews_total") or 0) > owner_reviews],
                key=lambda r: int(r.get("reviews_total") or 0),
                default=None,
            )
        ...
        if prev_sections and isinstance(prev_sections, dict) and isinstance(sections, dict) and created_id:
            ...
            def _apply_label_to_experience(exp: dict) -> dict:
                ...
                def _rt(text):
                    ...
                    text = text.replace("patient", _lsg).replace("Patient", _